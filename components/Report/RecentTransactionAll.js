        ...
        const fetchData = async () => {
            try {
                const response = await axios.get(
                    "https://ehouseapi20230817222213.azurewebsites.net/api/Contract/GetStatusAdminId?statusAdminId=true" ,{
                        headers: { 
                            Authorization: token,
                        } 
                    }
                                        ...
                                        contractData && contractData.map((item, i) => {
                                            ...
                                            return (
                                                <tr key={i}>
                                                    ...
                                                    <td>{datePart}</td>
                                                    <td><span className={`label badge-light-success`}>{item.statusAdminId ? ('Hoàn thành'):('Chờ duyệt')}</span></td>
